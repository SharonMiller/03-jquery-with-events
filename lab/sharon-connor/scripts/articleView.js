...
articleView.handleCategoryFilter = function () {
  ...
  $('#category-filter').on('change', function () {
    ...
    if ($(this).val()) {
      ...
      $('article').hide();
      $(`[data-category="${$(this).val()}"]`).fadeIn(750);
...
articleView.handleMainNav = function () {
  ...
  // So: You need to dynamically build a selector string with the correct ID, based on the data available to you on the .tab element that was clicked. DONE
...
articleView.setTeasers = function () {
  ...
  // Ideally, we'd attach this as just one event handler on the #articles section, and let it process (in other words... delegate) any .read-on clicks that happen within child nodes. DONE
};
